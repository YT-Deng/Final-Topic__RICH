...
sum2<-read.csv('C:/users/user/Documents/sum2017-2018.csv')
head(sum2)
sum2.1<- sum2 %>% 
  mutate(date = (paste(year,mon,day,sep="-") %>% ymd()))
head(sum2.1,3)
...
sum3<-read.csv('C:/users/user/Documents/sum2016-2017.csv')
head(sum3)
sum3.1<-sum3 %>%
  mutate(date = (paste(year,mon,day,sep="-") %>% ymd()))
head(sum3.1,3)
...
sum4<-read.csv('C:/users/user/Documents/sum2015-2016.csv')
head(sum4)
sum4.1<- sum4 %>% 
  mutate(date = (paste(year,mon,day,sep="-") %>% ymd()))
head(sum4.1,3)
