...
table1<-data.frame(Category=rep(c("train","high_speed_rail",
                                  "hotel"),each=15),
...
table1$Category<-factor(table1$Category,
                        levels=c("train","high_speed_rail",
                                   "hotel"),ordered=TRUE)
...
lab=paste0(round(COUNTRY/sum(COUNTRY)*100,2),"%")
